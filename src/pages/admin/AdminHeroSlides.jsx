...
export default function AdminHeroSlides() {
  ...
  async function refresh() {
    try {
      const rows = await getHeroSlides(true);
      setSlides(rows);
    } catch (err) {
  ...
  async function handleFile(file) {
    ...
    if (!isImage && !isVideo) {
      notify('Please upload an image or video file', 'error');
      return;
    }
    setUploading(true);
    try {
  ...
  async function handleSubmit(e) {
    ...
    if (!form.title || !form.media_url) {
      notify('Title and media are required', 'error');
      return;
    }
    setSaving(true);
    try {
      if (form.id) {
        await updateHeroSlide(form);
  ...
  async function togglePublish(slide) {
    try {
      await updateHeroSlide({ ...slide, is_published: slide.is_published ? 0 : 1 });
      refresh();
    } catch (err) {
      notify(err.message, 'error');
    }
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.1fr', gap: 24, alignItems: 'start' }}>
        ...
        <form className="card" onSubmit={handleSubmit}>
          ...
          >
            ...
            <input
              id="mediaInput" type="file" accept="image/*,video/*" hidden
